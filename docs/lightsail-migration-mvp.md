# Lightsail → NAS Site Migration — MVP Plan (veloso.dev)

Goal: pull a live site off an AWS Lightsail instance (WordPress is the assumed MVP case, since
that's what veloso.dev runs) and stand it back up on this NAS, using the Docker
Compose + Cloudflare Tunnel pattern this repo already deploys with. The long-term goal is a
generic "migrate any Lightsail-hosted site to a given NAS" command; veloso.dev is the first real
...
implementation: EC2 as a source, web servers other than what veloso.dev actually runs, databases
...
There are two concrete things this MVP needs to produce, both pulling from the same veloso.dev
Lightsail instance but landing very differently:

1. **veloso.dev → its own NAS deployment.** veloso.dev keeps being hosted, just moved off
...
2. **veloso.dev → `systemsnotsilos.com` (already running on the NAS, currently a fresh install
   with no content).** The user wants that site fully seeded with a clone of veloso.dev's DB,
   `wp-content` (files, plugins, themes), as a starting point — which they'll then manually diverge
   into a distinct tech-focused site (stripping the personal/CV content veloso.dev has, which is
...
mode from the start, rather than hardcoding veloso.dev twice:

```
synology-site migrate-from-lightsail --source-domain veloso.dev --target-domain veloso.dev --target-mode new-site --dry-run
synology-site migrate-from-lightsail --source-domain veloso.dev --target-domain veloso.dev --target-mode new-site --execute

synology-site migrate-from-lightsail --source-domain veloso.dev --target-domain systemsnotsilos.com --target-mode existing-site-replace --dry-run
synology-site migrate-from-lightsail --source-domain veloso.dev --target-domain systemsnotsilos.com --target-mode existing-site-replace --execute
```
...
- **`--execute`** — actually moves things: DB dump + restore, `wp-content` sync (local files via
  ...
  pattern already proven on this NAS for `systemsnotsilos-com`), Cloudflare cutover, verification.

## Confirmed so far

- veloso.dev's top-level domain runs **Nginx + WordPress + a SQL database (MySQL/MariaDB)** on
...
- Current Cloudflare DNS record(s) for veloso.dev, proxy (orange-cloud) status, and any existing
...
```
ssh-keygen -t ed25519 -f veloso-dev-migration -C "veloso-dev-migration-readonly"
```
...
`veloso-dev-migration-readonly`, with this inline policy:
...
`secrets/veloso-dev/aws.env` in this repo, following the same per-workspace pattern already used
...
## Current credential status (veloso.dev)

Tracking what's actually in `secrets/veloso-dev/` vs. still outstanding, updated as access is
provided:

| Item | Status |
|---|---|
| Cloudflare (veloso.dev zone) | **Already covered, confirmed twice** — the repo's default workspace (`.env`, `CF_ZONE_DOMAIN=veloso.dev`) already holds this. Also confirmed `CF_API_TOKEN`/`CF_ACCOUNT_ID` are byte-identical to `secrets/systemsnotsilos/cloudflare.env` (only `CF_ZONE_ID` differs, as expected per-zone) — same account/token already covers both domains. No separate `secrets/veloso-dev/cloudflare.env` needed. |
| SSH to the Lightsail instance | **Working.** The original address in `~/.ssh/config` turned out to be **stale** — a different, live instance entirely. The real instance was found by asking it directly (`curl` to the AWS metadata service from a browser SSH session) and cross-checked against its own nginx vhost for veloso.dev, now recorded in `secrets/veloso-dev/lightsail.env`. |
| S3 usage for veloso.dev media | **Confirmed not used**, directly (not just inferred from public HTML): `wp-content/plugins/` on the instance has no S3/offload plugin, and `wp-content/uploads/` is a plain local directory, 159M. Media migration is a plain `rsync`, no `aws s3 sync` needed. |
| AWS access | **Not needed for the MVP**, per the finding above. `secrets/veloso-dev/aws.env` still has the `velosodev` IAM user's Access Key ID on file in case a later phase wants Lightsail instance metadata, but there's no live blocker on it — no need to chase down its Secret Access Key right now. |
| NAS target workspace | Not yet decided — default workspace vs. a dedicated `secrets/veloso-dev/nas.env`. |

### What SSH discovery found on the instance (read-only)

- **Shared instance.** `/etc/nginx/sites-enabled` also serves a handful of other live third-party sites, plus disabled (`sites-available` only) configs for `systemsnotsilos.com` and a `veloso.dev.bkp`. This instance cannot be decommissioned once veloso.dev moves — every later step (file copy, DB dump, eventual cleanup) must stay scoped to veloso.dev's own paths only.
- **Document root:** `/var/www/html/veloso.dev/public/` — hand-rolled Nginx layout (not Bitnami), confirming the earlier guess. PHP 8.3 via `php8.3-fpm.sock`, TLS via Certbot/Let's Encrypt (`/etc/letsencrypt/live/veloso.dev/`) — this component is retired once Cloudflare Tunnel fronts the NAS, not migrated.
- **`blog.veloso.dev` resolved:** nginx has a *commented-out*, inactive server block that used to redirect `blog.veloso.dev`/`veloso.dev` → `https://www.blog.veloso.dev`. It's legacy/dead config, not a second live install — old post content just still references that hostname in stored image URLs (explains the earlier public-HTML finding). One current WordPress install, serving under `www.veloso.dev`.
...
### Extra hurdles specific to the `existing-site-replace` target (systemsnotsilos.com)

These don't apply to the veloso.dev `new-site` target (same domain, nothing to rewrite) but are
required for cloning onto a different domain:

- **Serialized-data-safe search-replace is mandatory, not optional.** veloso.dev's DB dump will
  have `veloso.dev` baked into `wp_options` (`siteurl`/`home`), post content, menus, widget
  ...
  length than `veloso.dev` (`systemsnotsilos.com` is longer, so this will always trigger). Needs a
  ...
  a plain SQL `REPLACE()`.
- **Don't touch the target's own `wp-config.php` DB credentials.** systemsnotsilos.com's containers
  already point at their own database name/user/host; the DB dump should be imported into that
  existing schema (drop-and-reimport its tables), not used to swap in veloso.dev's own DB
  connection details.
- **Jetpack and Google Site Kit need reconnecting after the clone.** Both tie their connection to a
  ...
  carry over cleanly to `systemsnotsilos.com`'s different domain — expect to reauthorize both
...
- **The "strip personal content" step is entirely manual and out of scope for this tool.** The plan
  here is just to land a faithful, working clone of veloso.dev on `systemsnotsilos.com` — removing
...
- **Back up systemsnotsilos.com's current (empty) install before overwriting it**, even though
...
Same pattern already used for other workspaces in this repo (e.g. `secrets/systemsnotsilos/`):

```
secrets/veloso-dev/cloudflare.env
...
Scope the token to the veloso.dev zone only (Cloudflare API tokens support per-zone scoping —
avoid the old-style global API key).

### 4. NAS target

Confirm whether veloso.dev deploys to the NAS's existing default workspace or gets its own
`secrets/veloso-dev/nas.env`, and confirm available disk headroom for the WordPress DB + uploads
...
Once SSH access, the AWS IAM access key, a Cloudflare Zone-Read token for veloso.dev, and the NAS
