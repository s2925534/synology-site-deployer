# Fixing SSH Access to the example.com Lightsail Instance

## The blocker

The key at `secrets/example-com/lightsail-ssh-key.pem` connects to the instance
(see `secrets/example-com/lightsail.env` for the address) and the *host key* now matches (you
...
2. Open the instance serving example.com (region `ap-southeast-2` / Sydney, per the existing SSH
   config).
...
4. In that browser terminal, run:
   ...
   echo '<paste the migration public key here>' >> ~/.ssh/authorized_keys
   chmod 600 ~/.ssh/authorized_keys
   ```
5. Confirm the user this landed under matches `ubuntu` (run `whoami` in that same terminal — if
   it says something else, e.g. `bitnami`, tell me and I'll update `secrets/example-com/lightsail.env`'s
...
2. Tell me its path (or copy it into `secrets/example-com/lightsail-ssh-key.pem` yourself, replacing
...
1. On your machine:
   ```
   ssh-keygen -t ed25519 -f /tmp/example-com-migration -N "" -C "example-com-migration-readonly"
   ```
   This creates `/tmp/example-com-migration` (private) and `/tmp/example-com-migration.pub` (public).
2. Print the public key so you can paste it in the next step:
   ```
   cat /tmp/example-com-migration.pub
   ```
...
4. Move the private key into this project's workspace and clean up the temp copy:
   ```
   mv /tmp/example-com-migration /Users/pedro/Documents/_Projects/synology-site-deployer/secrets/example-com/lightsail-ssh-key.pem
   rm /tmp/example-com-migration.pub
   ```
...
("Current credential status" table): whether `blog.example.com` is a second WordPress install or
